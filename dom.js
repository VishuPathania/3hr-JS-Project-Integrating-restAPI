async function saveToLocalStorage(event) {
  try {
    ...
    const response = await axios.post(
      "https://crudcrud.com/api/681cd8ed8f9040d3b693419350947c6a/candyData",
      obj
    );
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    for (let i = 0; i < response.data.length; i++) {
      ...
      await axios.delete(
        `https://crudcrud.com/api/681cd8ed8f9040d3b693419350947c6a/candyData/${response.data[i]._id}`
      );
    }
  } catch (err) {
    console.log(err);
...
async function deleteCandy(description) {
  try {
    await axios.delete(
      `https://crudcrud.com/api/681cd8ed8f9040d3b693419350947c6a   /candyData/${description}`
    );
